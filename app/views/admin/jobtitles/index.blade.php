...
<table id="jobtitles" class="table table-striped table-hover">
	...
	<tr>
		...
		<th class="col-md-1">{{{ Lang::get('admin/jobtitles/table.jobtitle_code') }}}</th>
		<th class="col-md-3">{{{ Lang::get('table.name') }}}</th>
		<th class="col-md-4">{{{ Lang::get('table.desc') }}}</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#create').click(function(e){
			e.preventDefault();
			getAjaxContent("{{ URL::to('admin/jobtitles/create') }}");
			$('#modal').modal();
		});
	});
