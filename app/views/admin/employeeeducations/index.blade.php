...
<table id="employeeeducations" class="table table-striped table-hover">
	<thead>
	<tr>
		<th class="col-md-1">{{{ Lang::get('table.status') }}}</th>
		<th class="col-md-4">{{{ Lang::get('admin/employeeeducations/table.employeeeducation_title') }}}</th>
		<th class="col-md-6">{{{ Lang::get('admin/employeeeducations/table.employeeeducation_detail') }}}</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		oTable = $('#employeeeducations').dataTable( {
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "{{ URL::to('admin/employeeeducations/' . $employee->id . '/data') }}",
	...
	$(document).ready(function() {
		...
		$('#create').click(function(e){
			e.preventDefault();
			getAjaxContent("{{ URL::to('admin/employeeeducations/' . $employee->id . '/create') }}");
			$('#modal').modal();
		});
	});
