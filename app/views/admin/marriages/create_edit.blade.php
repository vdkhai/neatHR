<form id="marriage" name="marriage" class="form-horizontal" method="post" action="@if (isset($marriage)){{ URL::to('admin/marriages/' . $marriage->id . '/edit') }} @else {{ URL::to('admin/marriages/create') }} @endif " autocomplete="off">
	...
	<div class="modal-body">
		...
		<div class="form-group">
			...
			<div class="col-md-10">
				<input class="form-control" type="text" name="name" id="name" value="{{{ Input::old('name', isset($marriage) ? $marriage->name : null) }}}" />
		...
		<div class="form-group">
			...
			<div class="col-md-10">
				<select class="form-control" name="published" id="published">
					<option value="1"{{{ (Input::old('published', isset($marriage) ? $marriage->published : 1) === 1 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.yes') }}}</option>
					<option value="0"{{{ (Input::old('published', isset($marriage) ? $marriage->published : 1) === 0 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.no') }}}</option>
				</select>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	var reload = false;
	@if ($mode == 'create')
		var url = '{{ URL::to("admin/marriages/create") }}';
	@else
	var url = '{{ URL::to("admin/marriages/" . $marriage->id . "/edit") }}';
	@endif
