<form id="certification" name="certification" class="form-horizontal" method="post" action="@if (isset($certification)){{ URL::to('admin/certifications/' . $certification->id . '/edit') }} @else {{ URL::to('admin/certifications/create') }} @endif " autocomplete="off">
	...
	<div class="modal-body">
		...
		<div class="form-group">
			...
			<div class="col-md-10">
				<textarea class="form-control" rows="3" name="description" id="description">{{{ Input::old('description', isset($certification) ? $certification->description : null) }}}</textarea>
		...
		<div class="form-group">
			...
			<div class="col-md-10">
				<select class="form-control" name="published" id="published">
					<option value="1"{{{ (Input::old('published', isset($certification) ? $certification->published : 1) === 1 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.yes') }}}</option>
					<option value="0"{{{ (Input::old('published', isset($certification) ? $certification->published : 1) === 0 ? ' selected="selected"' : '') }}}>{{{ Lang::get('general.no') }}}</option>
				</select>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	var reload = false;
	@if ($mode == 'create')
		var url = '{{ URL::to("admin/certifications/create") }}';
	@else
		var url = '{{ URL::to("admin/certifications/" . $certification->id . "/edit") }}';
	@endif
