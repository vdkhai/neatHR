...
<table id="countries" class="table table-striped table-hover">
	...
	<tr>
		...
		<th class="col-md-5">{{{ Lang::get('admin/countries/table.country_name') }}}</th>
		<th class="col-md-2">{{{ Lang::get('admin/countries/table.iso_code2') }}}</th>
		<th class="col-md-3">{{{ Lang::get('admin/countries/table.iso_code3') }}}</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#create').click(function(e){
			e.preventDefault();
			getAjaxContent("{{ URL::to('admin/countries/create') }}");
			$('#modal').modal();
		});
	});
