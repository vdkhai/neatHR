...
<table id="employeeskills" class="table table-striped table-hover">
	...
	<tr>
		...
		<th class="col-md-4">{{{ Lang::get('admin/employeeskills/table.employeeskill_title') }}}</th>
		<th class="col-md-6">{{{ Lang::get('admin/employeeskills/table.employeeskill_detail') }}}</th>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		oTable = $('#employeeskills').dataTable( {
			...
			"bProcessing": true,
			"bServerSide": true,
			"sAjaxSource": "{{ URL::to('admin/employeeskills/' . $employee->id . '/data') }}",
	...
	$(document).ready(function() {
		...
		$('#create').click(function(e){
			e.preventDefault();
			getAjaxContent("{{ URL::to('admin/employeeskills/' . $employee->id . '/create') }}");
			$('#modal').modal();
		});
	});
